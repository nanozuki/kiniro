...
describe('getStepIndexes', () => {
	it('builds scale indexes with optional half steps', () => {
		...
		expect(
			getStepIndexes({ ...scale, stepCount: 3, halfStepStart: true, halfStepEnd: true })
		).toEqual(['50', '100', '200', '300', '350']);
	});
...
describe('index-style mapping', () => {
	it('hides half-step settings when switching to ordinal style', () => {
		expect(
			createIndexStyleStructure({ ...scale, halfStepStart: true, halfStepEnd: true }, 'ordinal')
		).toEqual({
			stepCount: 5,
			indexStyle: 'ordinal',
			halfStepStart: false,
	...
	it('maps regular scale-step overrides to ordinal positions and drops half steps', () => {
		...
		expect(
			mapLightnessOverridesForIndexStyle(from, to, { '50': 1, '100': 0.9, '200': 0.7, '400': 0.2 })
		).toEqual({
			'1': 0.9,
			'2': 0.7,
			'4': 0.2
...
describe('reverse helpers', () => {
	it('maps overrides to opposite indexes', () => {
		expect(
			mapOverridesToReversedIndexes(scale, { '100': 'first', '200': 'second', '500': 'last' })
		).toEqual({
			'100': 'last',
			'400': 'second',
			'500': 'first'
