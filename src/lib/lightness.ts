...
export function computeStepLightness(
	...
	const anchors: StepAnchor[] = [
		...
		...indexes.slice(1, -1).flatMap((index, offset) => {
			...
			return value == null
				? []
				: [{ arrayIndex: offset + 1, position: positions[offset + 1], value }];
		}),
		{
			arrayIndex: indexes.length - 1,
			position: positions[indexes.length - 1],
...
export function buildSteps(structure: StepScaleStructure, values: StepScaleValues): Step[] {
	...
	return indexes.map((index, arrayIndex) => ({
		...
		controlled:
			arrayIndex === 0 || arrayIndex === indexes.length - 1 || cleanOverrides[index] != null
	}));
}
...
export function reverseStepScaleValues(
	structure: StepScaleStructure,
	values: StepScaleValues
): StepScaleValues {
	return {
...
function regularStepPosition(index: string, structure: StepScaleStructure): number | null {
	const numericIndex = Number(index);
	if (structure.indexStyle === 'ordinal')
		return numericIndex >= 1 && numericIndex <= structure.stepCount ? numericIndex - 1 : null;
...
function regularStepIndexAtPosition(
	position: number,
	structure: StepScaleStructure
): string | null {
