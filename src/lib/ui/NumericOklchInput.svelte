<script lang="ts">
	...
	let draft = $state('');
	const label = $derived(
		channel === 'lightness' ? 'Lightness' : channel === 'chroma' ? 'Chroma' : 'Hue'
	);
...
<label
	>{label}<input
		...
		inputmode="decimal"
		bind:value={draft}
		oninput={() => oncommit(normalizeChannelValue(channel, Number(draft || 0)))}
		onblur={() => oncommit(normalizeChannelValue(channel, Number(draft || 0)))}
	/></label
>
