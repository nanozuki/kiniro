...
// The next app model stores user-authored data only. Shared structure lives on the
// theme, while each variant owns only color and lightness values for that structure.
export function createEmptyAppState(): AppState {
	return { themes: [] };
}

// Creates the v1 starter theme described by the prototype: one variant, one empty
...
export function createDefaultRampValues(sourceColor: SourceColor = DEFAULT_SOURCE_COLOR): ColorRampValues {
	return {
...
export function createVariantValues(
	...
	for (const family of structure.families) {
		...
		families[family.id] = {
			stepScale: existingFamily?.stepScale ? clone(existingFamily.stepScale) : createDefaultStepScaleValues(),
			ramps
		};
	}
