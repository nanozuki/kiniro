...
export function validateThemeImport(json: string): ImportValidationResult {
	try {
		...
		if (!isRecord(value) || value.version !== EXPORT_VERSION) {
			return { ok: false, summary: 'Unsupported Kiniro export version.', details: 'Expected version: 1.' };
		}
		if (!Array.isArray(value.themes) || !value.themes.every(isThemeLike)) {
			return { ok: false, summary: 'Invalid theme export file.', details: 'The file must contain a themes array with theme payloads.' };
		}
		return { ok: true, file: { version: EXPORT_VERSION, themes: clone(value.themes) } };
	} catch (error) {
		return { ok: false, summary: 'Invalid JSON file.', details: error instanceof Error ? error.message : 'Unable to parse JSON.' };
	}
}

export function applyThemeImport(existingThemes: readonly Theme[], importedThemes: readonly Theme[], choices: readonly ImportThemeChoice[]): ImportApplyResult {
