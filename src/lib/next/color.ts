...
import type { GamutPreview, OklchChannel, OklchColor, SourceColor, SourceColorFormat } from './model';
...
export function parseSourceColor(input: string, format = inferSourceFormat(input)): SourceColor | null {
	try {
...
export function createSourceColor(oklch: OklchColor, format: SourceColorFormat = 'oklch'): SourceColor {
...
export function sourceColorFromRgb(rgb: RgbColor, format: SourceColorFormat = 'rgb'): SourceColor {
	return createSourceColor(colorToOklch(new Color('srgb', [rgb.red / 255, rgb.green / 255, rgb.blue / 255])), format);
}

export function sourceColorFromHsl(hsl: HslColor, format: SourceColorFormat = 'hsl'): SourceColor {
	return createSourceColor(colorToOklch(new Color('hsl', [hsl.hue, hsl.saturation, hsl.lightness])), format);
}
...
export function getGamutStatus(oklch: OklchColor, gamut: GamutPreview): GamutStatus {
	...
	if (outOfSelectedGamut) {
		warning = inP3 ? 'Outside sRGB. Preview is clamped.' : 'Outside sRGB and P3. Preview is clamped.';
	}
...
function serializeRgb(color: Color): string {
	const [red, green, blue] = color.to('srgb').toGamut({ space: 'srgb' }).coords.map((value) =>
		Math.round(clamp(finiteOrZero(value), 0, 1) * 255)
	);
...
function serializeP3(color: Color): string {
	const [red, green, blue] = color.to('p3').toGamut({ space: 'p3' }).coords.map((value) =>
		formatUnit(finiteOrZero(value))
	);
