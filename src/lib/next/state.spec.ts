...
import { createNextAppManager } from './state';
...
describe('NextAppManager selection and UI state', () => {
	it('repairs invalid selection to a valid screen', () => {
		const theme = createDefaultTheme({ id: 'theme', variantId: 'variant' });
		const manager = createNextAppManager({ data: { themes: [theme] }, ui: { selection: { themeId: 'missing', variantId: 'missing' }, workspaceTab: 'cssVariables' } });
...
describe('NextAppManager theme and variant operations', () => {
	it('adds, renames, and deletes themes with selection repair', () => {
		const manager = createNextAppManager({ idFactory: ids('theme-1', 'variant-1', 'family-1', 'theme-2', 'variant-2', 'family-2') });
	...
	it('copies variant values and keeps shared structure', () => {
		const manager = createNextAppManager({ data: { themes: [createDefaultTheme({ id: 'theme', variantId: 'main' })] }, idFactory: ids('variant-2') });
...
describe('NextAppManager family and step operations', () => {
	it('adds and deletes shared families across variants', () => {
		const manager = createNextAppManager({ data: { themes: [createDefaultTheme({ id: 'theme', variantId: 'main' })] }, idFactory: ids('variant-2', 'family-2') });
		manager.addVariant('Dawn');
		const family = manager.addFamily();

		expect(family?.name).toBe('Family 2');
		expect(manager.selectedTheme?.variants.every((variant) => Boolean(variant.values.families['family-2']))).toBe(true);

		manager.deleteFamily('family-2');
		expect(manager.selectedTheme?.structure.families.map((item) => item.id)).toEqual(['family-1']);
		expect(manager.selectedTheme?.variants.every((variant) => variant.values.families['family-2'] == null)).toBe(true);
	});
	...
	it('updates step structure globally and step values for the selected variant only', () => {
		...
		expect(manager.selectedTheme?.structure.families[0].stepScale).toMatchObject({ indexStyle: 'ordinal', stepCount: 5 });
...
describe('NextAppManager ramp and swatch operations', () => {
	it('adds, edits, and deletes ramps across shared structure and variant values', () => {
		const manager = createNextAppManager({ data: { themes: [createDefaultTheme({ id: 'theme', variantId: 'main' })] }, idFactory: ids('variant-2', 'ramp-1') });
		manager.addVariant('Dawn');
		manager.selectVariant('main');

		const ramp = manager.addRamp('family-1', source, 'Ramp');
		manager.renameRamp('ramp-1', 'Ramp');
		manager.setRampSourceColor('family-1', 'ramp-1', createSourceColor({ lightness: 0.4, chroma: 0.2, hue: 120 }, 'hex'));

		expect(ramp?.name).toBe('Ramp');
		expect(manager.selectedTheme?.variants.every((variant) => Boolean(variant.values.families['family-1'].ramps['ramp-1']))).toBe(true);
		expect(manager.selectedVariant?.values.families['family-1'].ramps['ramp-1'].sourceColor.format).toBe('hex');
	...
	it('sets and resets swatch overrides and reverses selected variant overrides only', () => {
		const manager = createNextAppManager({ data: { themes: [createDefaultTheme({ id: 'theme', variantId: 'main' })] }, idFactory: ids('ramp-1') });
		...
		expect(manager.selectedVariant?.values.families['family-1'].ramps['ramp-1'].swatchOverrides).toEqual({ '300': { chroma: 0.2 } });

		manager.reverseFamilyLightness('family-1');
		expect(manager.selectedVariant?.values.families['family-1'].ramps['ramp-1'].swatchOverrides).toEqual({ '700': { chroma: 0.2 } });
		manager.resetSwatchColor('family-1', 'ramp-1', '700');
		expect(manager.selectedVariant?.values.families['family-1'].ramps['ramp-1'].swatchOverrides).toEqual({});
	});
});
