<script lang="ts">
	import { createSourceColor, parseSourceColor, sourceColorFromHsl, sourceColorFromRgb } from './color';
	...
	<div class="dialog-body">
		...
		<div role="group" aria-label="Source color format" class="segments">
			{#each ['oklch', 'hex', 'rgb', 'hsl'] as option}
				<button type="button" aria-pressed={format === option} onclick={() => switchFormat(option as SourceColorFormat)}>{option.toUpperCase()}</button>
			{/each}
		</div>

		{#if format === 'oklch'}
			<label>Lightness <input aria-label="Lightness" type="number" min="0" max="1" step="0.01" value={sourceColor?.oklch.lightness ?? 0} oninput={(event) => updateOklch('lightness', numeric(event))} /></label>
			<label>Chroma <input aria-label="Chroma" type="number" min="0" max="0.37" step="0.01" value={sourceColor?.oklch.chroma ?? 0} oninput={(event) => updateOklch('chroma', numeric(event))} /></label>
			<label>Hue <input aria-label="Hue" type="number" min="0" max="360" step="1" value={sourceColor?.oklch.hue ?? 0} oninput={(event) => updateOklch('hue', numeric(event))} /></label>
		{:else if format === 'hex'}
			<label>Hex <input aria-label="Hex" type="text" value={textDraft} oninput={(event) => updateText((event.currentTarget as HTMLInputElement).value)} /></label>
			<input aria-label="Native color picker" type="color" value={sourceColor?.serialized.startsWith('#') ? sourceColor.serialized : '#000000'} oninput={(event) => updateText((event.currentTarget as HTMLInputElement).value)} />
		{:else if format === 'rgb'}
			<label>Red <input aria-label="Red" type="number" min="0" max="255" value={parseRgbDraft(textDraft).red} oninput={(event) => updateRgb('red', numeric(event))} /></label>
			<label>Green <input aria-label="Green" type="number" min="0" max="255" value={parseRgbDraft(textDraft).green} oninput={(event) => updateRgb('green', numeric(event))} /></label>
			<label>Blue <input aria-label="Blue" type="number" min="0" max="255" value={parseRgbDraft(textDraft).blue} oninput={(event) => updateRgb('blue', numeric(event))} /></label>
		{:else}
			<label>Hue <input aria-label="Hue" type="number" min="0" max="360" value={parseHslDraft(textDraft).hue} oninput={(event) => updateHsl('hue', numeric(event))} /></label>
			<label>Saturation <input aria-label="Saturation" type="number" min="0" max="100" value={parseHslDraft(textDraft).saturation} oninput={(event) => updateHsl('saturation', numeric(event))} /></label>
			<label>Lightness <input aria-label="Lightness" type="number" min="0" max="100" value={parseHslDraft(textDraft).lightness} oninput={(event) => updateHsl('lightness', numeric(event))} /></label>
		{/if}
...
<style>
	.dialog-body { display: grid; gap: 0.75rem; }
	.segments, .actions { display: flex; gap: 0.5rem; flex-wrap: wrap; }
...
<script lang="ts" module>
	...
	function validateName(value: string, existingNames: string[]) {
		...
		if (existingNames.some((name) => name.toLowerCase() === trimmed.toLowerCase())) return 'Ramp name must be unique.';
		return '';
	}
