...
export function calculateContrastRatio(foreground: OklchColor, background: OklchColor, gamut: GamutPreview): number {
...
export function getContrastResult(foreground: OklchColor, background: OklchColor, gamut: GamutPreview): ContrastResult {
...
export function getContrastTargets(palette: GeneratedVariantPalette): ContrastTarget[] {
	return palette.families.flatMap((family) =>
		family.ramps.flatMap((ramp) => ramp.swatches.map((swatch) => targetFromSwatch(family.id, ramp, swatch)))
	);
}

export function getDefaultContrastTargets(palette: GeneratedVariantPalette): ContrastDefaults | null {
	const ramps = palette.families.flatMap((family) =>
		family.ramps.map((ramp) => ({ familyId: family.id, ramp })).filter(({ ramp }) => ramp.swatches.length > 0)
	);
	if (ramps.length === 0) return null;

	const foregroundRamp = ramps[0];
	const backgroundRamp = ramps[1] ?? ramps[0];
	const foregroundSwatch = foregroundRamp.ramp.swatches[Math.min(1, foregroundRamp.ramp.swatches.length - 1)];
	const backgroundSwatch = backgroundRamp.ramp.swatches[Math.max(0, backgroundRamp.ramp.swatches.length - 2)];
...
function targetFromSwatch(familyId: string, ramp: GeneratedColorRamp, swatch: GeneratedSwatch): ContrastTarget {
	return {
		familyId,
		rampId: ramp.id,
		rampName: ramp.name,
