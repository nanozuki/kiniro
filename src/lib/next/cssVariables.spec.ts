...
describe('exportCssVariables', () => {
	...
	it('preserves family, ramp, and step order', () => {
		...
		theme.structure.families[0].ramps.push({ id: 'base', name: 'base' }, { id: 'text', name: 'text' });
		const variant = theme.variants[0];
		variant.values.families['family-1'].ramps.base = createDefaultRampValues();
	...
	it('includes comments for empty families and blank lines between groups', () => {
		...
		theme.variants[0].values.families.empty = {
			stepScale: { lightnessStart: 0.9, lightnessEnd: 0.1, lightnessOverrides: {}, reversed: false },
			ramps: {}
		};
