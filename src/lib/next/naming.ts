...
export type NameValidationError = 'empty-display-name' | 'empty-css-name' | 'duplicate-name' | 'duplicate-css-name';
...
export function validateName(name: string, existingNames: readonly NamedItem[] = [], options: UniqueNameOptions = {}): NameValidationResult {
	...
	else if (hasDisplayNameCollision(normalizedName, existingNames, options.exclude)) error = 'duplicate-name';
	else if (hasCssNameCollision(cssName, existingNames, options.exclude)) error = 'duplicate-css-name';
...
export function ensureUniqueName(
	...
	const normalizedName = normalizeDisplayName(name);
	const baseName = normalizedName.length > 0 && sanitizeCssName(normalizedName).length > 0 ? normalizedName : fallbackBase;
...
function findNumberedName(
	...
	const usableBase = sanitizeCssName(normalizedBase).length > 0 ? normalizedBase : options.fallbackBase || 'Name';
...
function comparableNames(items: readonly NamedItem[], exclude: UniqueNameOptions['exclude']): string[] {
	return items.filter((item) => !isExcluded(item, exclude)).map((item) => normalizeDisplayName(item.name));
}

function comparableCssNames(items: readonly NamedItem[], exclude: UniqueNameOptions['exclude']): string[] {
	return items.filter((item) => !isExcluded(item, exclude)).map((item) => sanitizeCssName(item.name));
}
