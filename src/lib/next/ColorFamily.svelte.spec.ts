...
describe('ColorFamily', () => {
	...
	it('shows the shared-structure warning only when multiple variants exist', async () => {
		...
		const family = theme.structure.families[0];
		const { rerender } = render(ColorFamily, { family, variant: theme.variants[0], variantCount: 1 });

		await expect.element(page.getByText('Family structure is shared by all variants in this theme.')).not.toBeInTheDocument();
		await rerender({ family, variant: theme.variants[0], variantCount: 2 });
		await expect.element(page.getByText('Family structure is shared by all variants in this theme.')).toBeInTheDocument();
	});
});
