...
function themes() {
	const first = createDefaultTheme({ id: 'theme-1', name: 'Rose Pine', variantId: 'main', variantName: 'Main' });
	first.variants.push(createThemeVariant(first.structure, { id: 'moon', name: 'Moon' }));
	const second = createDefaultTheme({ id: 'theme-2', name: 'Gruvbox', variantId: 'dark', variantName: 'Dark' });
	return [first, second];
}
...
describe('ThemeManager', () => {
	it('renders theme and variant navigation and selection callbacks', async () => {
		...
		render(ThemeManager, { themes: themes(), selectedThemeId: 'theme-1', selectedVariantId: 'main', onselecttheme, onselectvariant });

		await expect.element(page.getByRole('button', { name: 'Rose Pine' })).toHaveAttribute('aria-current', 'true');
	...
	it('delegates create actions', async () => {
		...
		render(ThemeManager, { themes: themes(), selectedThemeId: 'theme-1', selectedVariantId: 'main', onaddtheme, onaddvariant });
	...
	it('renames and deletes selected theme and variant', async () => {
		...
		render(ThemeManager, { themes: themes(), selectedThemeId: 'theme-1', selectedVariantId: 'main', onrenametheme, onrenamevariant, ondeletetheme, ondeletevariant });
