...
describe('ContrastChecker', () => {
	...
	it('updates target selections and swaps colors', async () => {
		...
		await expect.element(page.getByLabelText('Foreground color')).toHaveValue('family-1:ramp-1:900');
		await expect.element(page.getByLabelText('Background color')).toHaveValue('family-1:ramp-2:100');
	});
