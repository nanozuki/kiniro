<script lang="ts">
	import { exportThemes, validateThemeImport, type ImportConflictChoice, type ThemeExportFile } from './importExport';
	...
	let exportSelection = $derived(themes.filter((theme: Theme) => selectedThemeIds.includes(theme.id)));
	...
	function toggleTheme(themeId: string, checked: boolean) {
		selectedThemeIds = checked ? [...selectedThemeIds, themeId] : selectedThemeIds.filter((id) => id !== themeId);
	}
	...
	async function readImport(event: Event) {
		...
		importChoices = result.file.themes.map((theme) => ({ themeId: theme.id, conflict: hasConflict(theme) ? 'rename' : undefined }));
	}
	...
	function setConflict(themeId: string, conflict: ImportConflictChoice) {
		importChoices = importChoices.map((choice) => (choice.themeId === themeId ? { ...choice, conflict } : choice));
	}
...
{#if exportOpen}
	<section role="dialog" aria-modal="true" aria-label="Export themes">
		...
		{#each themes as theme}
			<label><input type="checkbox" checked={selectedThemeIds.includes(theme.id)} onchange={(event) => toggleTheme(theme.id, event.currentTarget.checked)} /> {theme.name}</label>
		{/each}
	...
	<section role="dialog" aria-modal="true" aria-label="Import themes">
		...
		{#if importFile}
			{#each importFile.themes as theme}
				<div class="choice">
					<label><input type="checkbox" checked={importChoices.some((choice) => choice.themeId === theme.id)} onchange={(event) => choose(theme.id, event.currentTarget.checked)} /> {theme.name}</label>
					{#if hasConflict(theme)}
						<label>Conflict
							<select aria-label={`Conflict choice for ${theme.name}`} value={importChoices.find((choice) => choice.themeId === theme.id)?.conflict ?? 'rename'} onchange={(event) => setConflict(theme.id, event.currentTarget.value as ImportConflictChoice)}>
