<script lang="ts">
	import { getContrastResult, getContrastTargets, getDefaultContrastTargets, type ContrastTarget } from './contrast';
	import { getPreviewColor } from './color';
	import type { GamutPreview } from './model';
	import type { GeneratedVariantPalette } from './palette';

	let { palette, gamutPreview = 'srgb' } = $props<{ palette: GeneratedVariantPalette; gamutPreview?: GamutPreview }>();
	...
	let result = $derived(foreground && background ? getContrastResult(foreground.color, background.color, gamutPreview) : null);
...
<section aria-label="Contrast Checker" class="contrast-checker">
	...
	{#if foreground && background && result}
		<div class="showcase" style={`color: ${getPreviewColor(foreground.color, gamutPreview).css}; background: ${getPreviewColor(background.color, gamutPreview).css}`}>
			<strong>Showcase text</strong>
			<span>{foreground.label} on {background.label}</span>
		</div>
		<div class="controls">
			<label>Foreground
				<select value={key(foreground)} onchange={(event) => (foregroundKey = event.currentTarget.value)} aria-label="Foreground color">
					{#each targets as target}<option value={key(target)}>{target.label}</option>{/each}
				</select>
			</label>
			<label>Background
				<select value={key(background)} onchange={(event) => (backgroundKey = event.currentTarget.value)} aria-label="Background color">
					{#each targets as target}<option value={key(target)}>{target.label}</option>{/each}
				</select>
			</label>
			...
			<tbody>
				{#each result.checks as check}
					<tr>
						...
						<td>{check.enhancedAAA ? `${check.enhancedAAA.label} (${check.enhancedAAA.threshold}:1)` : 'N/A'}</td>
					</tr>
				{/each}
			</tbody>
		</table>
...
<style>
	.contrast-checker { display: grid; gap: 1rem; }
	.showcase { border: 1px solid currentColor; border-radius: 0.5rem; min-block-size: 8rem; padding: 1rem; display: grid; align-content: center; }
