...
export function generateVariantPalette(theme: Theme, variant: ThemeVariant): GeneratedVariantPalette {
	return {
		themeId: theme.id,
...
export function generateFamily(family: ColorFamilyStructure, variant: ThemeVariant): GeneratedColorFamily {
...
export function generateRamp(
	...
	return {
		...
		name: ramp.name,
		sourceColor,
		swatches: steps.map((step) => generateSwatch(ramp.name, step, sourceColor, values?.swatchOverrides[step.index]))
	};
}
